...
export default function Edit({ agency }: { agency: Agency }) {
    return (
        <AppLayout
            breadcrumbs={[{ title: 'Platform' }, { title: 'Agencies', href: index().url }, { title: 'Edit agency' }]}
        >
            <PageHeader title="Edit agency" description={agency.name} />
            <Card>
                <CardContent>
                    <Form {...update.form(agency)} className="grid gap-4">
                        {({ errors, processing }) => (
                            <>
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <Input id="name" name="name" defaultValue={agency.name} maxLength={120} required />
