...
**The largest design consequence in this file.** The overtime threshold is not a constant — 8 by default, 12 under a compliant CWW — **and it is two numbers, not one**: a daily threshold and a weekly ceiling of 48. A single `overtime_after` setting cannot express it. CSC's compressed week has no premium exemption, so this is a genuine per-agency rule.
