...
| 9 | **`workdays.night` is one scalar and cannot serve the no-stacking rule.** Section D records that the differential does not stack on overtime (JC 1 s. 2023 Annex C Example D); a single night total mixes regular-shift and overtime night minutes, so a CSC agency cannot extract the figure it is owed | M6, recorded in `../design/06-attendance.md` "Open before Milestone 6" item 3 |
