...
### Open before Milestone 6

Four defects a two-lineage adversarial review found on 2026-09-10, all in the rules above and all confirmed against the committed text. They are recorded rather than fixed because the fix is a rework of this computation model, not a wording correction; decision 34 states that. **A private-sector agency cannot be billed correctly until 1 and 2 are resolved.** Item 5 is a different kind of entry — not a defect in these rules but work this milestone owes, handed over by decision 35 because Milestone 6 is the first point at which it can be built at all.

1. **Holiday and rest-day work collapses into `excess`, and the first eight hours of it are not overtime.** Under Labor Code Arts. 93–94 the first eight hours worked on a rest day or a regular holiday are *regular hours at a premium* — 130% and 200% — and only hours beyond eight are overtime. But an `Off` shift has empty slots (`04-scheduling.md`) and a non-working holiday expects no work (`05-calendar.md`), so rule 5's per-slot overlap has nothing to overlap: `worked` computes to **0** and every actual minute lands in `excess`. Rule 6 then says compensable overtime is `excess ∩ Overtime authority` — and an `Overtime` authority is a JC 2 s. 2015 government instrument that a private employer does not have. So eight hours of lawful 200% holiday work sit in a bucket whose only consumer is gated on a document that does not exist for that agency. There is no column for premium-rate regular hours.
2. **No ISO-week accumulator, while a weekly ceiling is already a setting.** DA 02-04 makes work beyond **48 hours a week** overtime even when no single day exceeds twelve, so a fifth eight-hour day after a 4×12 compressed week is entirely weekly overtime. `settings.overtime_after_weekly` exists for exactly this (`../reference/dole-rules.md` section H) but nothing can compute it: workdays are strictly daily (`UNIQUE (employee_id, date)`) and the ledger's only periods are `first`, `second` and `full`. ISO weeks appear in rule 8 for fallback-shift resolution and nowhere in accumulation.
3. **`workdays.night` cannot separate regular night minutes from overtime night minutes.** It is one `smallint`. The two regimes diverge on precisely that split: under CSC-DBM-GCG JC 1 s. 2023 Annex C the differential does **not** stack on overtime hours, while under Labor Code Art. 86 and RA 7305 §18(b) a premium does attach to overtime rendered in the window. A single total is therefore unusable as-is by a civil-service agency, which needs the regular-shift portion alone.
4. **Cross-midnight attribution of *day status* is unstated.** "Across midnight and month end" gives the workday of date D every punch of the shift starting on D, and that is right for ownership. But `status` is a single value per row, so a shift running 22:00 Sunday into a 06:00 Monday that is a regular holiday has no way to record that 360 of its minutes fell on Monday's holiday: Sunday's row is `present`, and Monday's row cannot claim the punches because Sunday's already did. The raw facts survive — `punches.actual_at` against dated holidays derives the split, the same escape hatch decision 33 relies on — so this is a deriver and attribution-policy gap, not lost data. What is missing is a stated policy, frozen historically like the shift is. Note the legal premise is a **default, not an invariant**: holidays run midnight to midnight, but a contract, company policy or CBA may attribute a whole night shift to the evening it began, and that language was read only in a non-primary mirror.
5. **A locked or attested ledger month does not yet freeze the deployment ranges it was computed from** — owed by decision 35, and buildable only here. Correcting a deployment is a `DELETE` and re-create, not a re-date, and there is deliberately no `PATCH`; until this milestone that is unconditionally safe, because nothing reads a deployment range yet. Once ledgers exist it is not: decision 30 makes visibility a predicate over deployment ranges *by overlap*, so deleting or re-dating a range retroactively changes who could see, attest or correct a month that may already be locked and signed. What is owed is a trigger refusing **any write** — insert, re-date or delete — to a deployment overlapping a locked or attested month. It must be a trigger and cannot be a foreign key, for two independent reasons: the relationship is an overlap rather than a reference, and `attestations` hang off `ledgers`, which carry no `deployment_id` by decision 30 and must not acquire one. `deployments` has exactly one incoming FK, itself, and decision 35 fixes that permanently — so this guard has nowhere declarative to live. The dependency runs the other way from the rest of this list: items 1 to 4 are defects in the rules above, while this one is a constraint the *organisation* tables are waiting on these tables to make possible.


6. **Two exemptions covering one day have no stated winner, and `workdays.exemption_id` holds exactly one.** The schema permits this on purpose (05-calendar.md rule 4): a two-hour pass slip in the morning and a CTO in the afternoon are one ordinary day, and a whole-day exemption may sit inside a longer leave after a correction. But nothing declares which row a workday stamps, so a deriver picking arbitrarily could stamp the `personal` slip that excuses nothing over the `leave` that excuses everything, and the DTR would show a day of absence the employee is not liable for. What is owed is a stated precedence — plausibly `excused()` first, then the narrower window, then the earlier `approved_at` — frozen the way the shift snapshot is. Named by the first adversarial review of Milestone 4, which was asked whether the absent exclusion constraint left a hole and correctly answered that it leaves this one. Note the stamp is a *summary*, not the authority: every exemption row survives, so a corrected precedence recomputes rather than loses anything.
7. **`local` holidays have no stated effect on a workday.** Rule 1 of 05-calendar.md says what `regular`, `special` and `working` do and is silent on the fourth value of the type set, which decision 19's DDL nonetheless requires. Philippine practice treats an LGU holiday as a special non-working day unless the ordinance says otherwise, so the likely answer is "as `special`" — but a deriver that falls through a `match` on an unstated case either raises or silently treats the day as ordinary, and the second is a wrong DTR. Either state the mapping in rule 1 or give `HolidayType` a behavioural method the way `ExemptionType::excuses()` is the one place deciding that question. Surfaced while implementing `holidays`: the reviewer's attack on the absent `local`-not-platform trigger was answered by "`type` is a rate and `agency_id` is the scope", and checking that answer against rule 1 showed the rate for this one is never given.
