...
23. Decided 2026-09-09: CS Form 48 places each punch by its clock time. A punch prints in the column its own clock time falls in, AM before 12:00 and PM from 12:00, with the next-day marker where the punch belongs to a later date, so a single-pair night shift of 22:00 to 06:00 prints as a PM arrival and an AM departure marked `⁺¹`. This supersedes the wording in 06-attendance.md that a one-pair shift prints its arrival in the first column and its departure in the last, which put a 22:00 arrival in the AM column. Say if not.
