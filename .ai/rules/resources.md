...
A count added by `withCount` is exposed with `whenCounted()`, so the key is absent everywhere the query did not ask for it and a missing count can never read as zero. Its TypeScript home is the consuming page's own row interface (`AgencyRow`, `UnitRow`), never the shared `Agency`/`Unit` interface in `types/index.d.ts`.

Alias the count for what it answers, not for the relation it walks: `withCount(['deployments as people_count' => fn ($q) => $q->whereNull('ends')])` is a headcount, and `units/index` also asks for the unaliased `deployments_count` because "who is here now" and "has anyone ever been here" are two different questions and one number cannot serve both.
