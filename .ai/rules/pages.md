...
Server side, whitelist any filter whose value names a row or a vocabulary (`workgroup`, `tag`) against what the tenant actually has and report the dropped value as unset in `filters`, the way AgencyController::index whitelists `sort`. A mangled query string must not leave the list filtered by a value the picker cannot display.

Choosing a workgroup means that workgroup **and everything under it** (01-organization.md rule 4, `Workgroup::descendants()`): a department whose people all sit in its divisions would otherwise answer with nothing. Say so in the control's accessible name ("15 in Administrative Division and below") — and count the number over the subtree too, not just the workgroup's own row. A count whose label and destination both mean the subtree has to *be* the subtree's: `workgroups/index` shipped a direct `withCount` under that label, and MEASURED on the seeded Demo Agency the row displayed **6** while the link beside it reported **15** (Office of the Executive Director: 5 against 28). `WorkgroupController::rollUpPeople()` rolls `people_count` up over the flat list the query already returned — one O(n) pass, no extra query — while `deployments_count` stays direct, because that one guards a RESTRICT on the workgroup's own rows.

## The workgroup tree: indented table rows with hairline guides, not a widget
A tree is composed from §5.13's table, not from a new component: `lib/workgroups.ts`'s `flattenWorkgroups()` turns the flat `parent_id` list into tree order and hands each row `depth`, `last`, `children` and `guides`.
...
Bound the first column's width. With only the last column sized, the Workgroup column took 538 of 1210 and left a 280px void before the next label. Let a later text column be the flexible one instead.
