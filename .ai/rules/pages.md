...
Choosing a unit means that unit **and everything under it** (01-organization.md rule 4, `Unit::descendants()`): a department whose people all sit in its divisions would otherwise answer with nothing. Say so in the control's accessible name ("15 in Administrative Division and below") — and count the number over the subtree too, not just the unit's own row. A count whose label and destination both mean the subtree has to *be* the subtree's: `units/index` shipped a direct `withCount` under that label, and MEASURED on the seeded Demo Agency the row displayed **6** while the link beside it reported **15** (Office of the Executive Director: 5 against 28). `UnitController::rollUpPeople()` rolls `people_count` up over the flat list the query already returned — one O(n) pass, no extra query — while `deployments_count` stays direct, because that one guards a RESTRICT on the unit's own rows.
